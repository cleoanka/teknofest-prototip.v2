...
Actions) mevcuttur. Ayrılmış (held-out) tespitte plaka mAP50 \textbf{0,983}; üç gerçek test videosunda
(sınıf başına N=1) davranış makro-F1 \textbf{1,0} (sıfır çapraz yanlış-pozitif) ve plaka okuma
\textbf{3/3 tam-eşleşme} (CER, karakter hata oranı 0,0; sıfır yanlış-onay) elde edilmiş; hedef GPU'da
ölçülen \textbf{12--15 FPS} iş-hacmiyle, kamera akışı kare-örnekleme ile gerçek-zamanlıya yakın
...
\textbf{\%100}'dür. \textit{Dürüst bağlam:} sınıf başına destek N=1;
...
\begin{table}[H]\centering\small
\caption{Davranış tespiti — üç gerçek test videosunda video-düzeyi P/R/F1 (her hücre P/R/F1). \textbf{Aşama-1 profili} yalnız ana dedektörü değiştirir; \tcode{phone}/\tcode{smoking} fine-tune gerektirmeyen Aşama-2a poz-geometrisiyle, \tcode{swerving} dedektörden bağımsız kinematikle üretilir (stok COCO'da bu sınıflar yoktur).}\label{tab:beh}
...
\begin{figure}[ht]\centering
\includegraphics[width=0.45\textwidth]{fig_plaka_ab.png}
...
çıkar ve aynı A/B harness'i daha agresif bir OFF parametresiyle bu yönlülüğü yeniden-üretilebilir
biçimde sınamaya hazırdır; nicel delta henüz ölçülmediğinden bu raporda sayısal bir iyileşme iddia
edilmez. Dolayısıyla QoD'nin katkısı, sabit ölçülmüş tek bir sayı değil,

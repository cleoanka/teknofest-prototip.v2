...
\vfill
{\small Bu rapordaki tüm doğruluk sayıları, depo ölçüm artefaktlarından otomatik türetilmiş
ve yeniden üretilebilirdir (ayrıntı: Bölüm 4.7 Kanıt İzi).}
\end{center}
\newpage
...
Actions) mevcuttur. Held-out ölçümlerde plaka tespiti mAP50 \textbf{0,983}, davranış tespiti
makro-F1 \textbf{1,0} (sıfır çapraz yanlış-pozitif), plaka okuma \textbf{3/3 tam-eşleşme} (CER 0,0,
sıfır yanlış-onay) ve hedef GPU'da \textbf{12--15 FPS} gerçek-zamanlı işleme elde edilmiştir.
...
(3.104), \tcode{smoking} (557) ve \tcode{phone} (659, sentetik render). İndirilen bu dört set
\tcode{data/processed/} altında işlenmiş olarak bulunur; kaynak/lisans/sınıf-eşlemesi Bölüm 5
kaynakçasında, ek/büyük set çekme planı ise \tcode{train/datasets.yaml} manifestinde belgelidir.
...
\includegraphics[width=0.40\textwidth]{fig_veri_dengesi.png}
...
\begin{figure}[ht]\centering
\includegraphics[width=0.44\textwidth]{fig_split.png}
...
\begin{figure}[ht]\centering
\includegraphics[width=0.45\textwidth]{fig_mimari.png}
...
CLAHE + gamma parlatma ve küçük kırpık için süper-çözünürlük (gece/los koşul telafisi).
...
\begin{table}[H]\centering\small
\caption{Held-out tespit doğruluğu (özel modeller: \tcode{weights/custom\_*\_s.metrics.json}; stok: COCO val2017). $^{\dagger}$\,\tcode{license\_plate} üretimde varsayılan LP dedektörüdür; \tcode{seatbelt}/\tcode{smoking} fine-tune held-out sonuçlarıdır (üretim hattında opsiyonel/devre-dışı).}\label{tab:map}
...
\begin{figure}[ht]\centering
\begin{minipage}{0.46\textwidth}\centering\includegraphics[width=\textwidth]{fig_tespit_map.png}\end{minipage}\hfill
\begin{minipage}{0.46\textwidth}\centering\includegraphics[width=\textwidth]{fig_custom_prf1.png}\end{minipage}
...
\textbf{\%100}'dür. \textit{Dürüst bağlam:} sınıf başına destek N=1 (üç videoda birer pozitif örnek);
istatistiksel güç held-out mAP setlerinden (Tablo~\ref{tab:map}) gelir, video seti yalnız uçtan-uca
doğrulamadır. Emniyet kemeri (\tcode{seatbelt}) yalnızca held-out mAP (0,895) ile sınanmıştır; üç
...
\begin{figure}[ht]\centering
\includegraphics[width=0.42\textwidth]{fig_plaka_ab.png}
...
QoD entegrasyonu, A/B harness ile \textbf{yeniden-üretilebilir} biçimde ölçülür: aynı kaynak, QoD OFF
(düşük çözünürlük/bant simülasyonu) ve QoD ON (tam çözünürlük) ile koşulup plaka, küçük-nesne ve tespit
metrikleri kıyaslanır (\tcode{aura.eval --qod-comparison}). \textbf{Dürüst not (kritik):} mevcut
kontrollü koşuda (\tcode{eval\_results/report.json}) OFF baseline'ı zaten yeterli kalitede olduğundan
ölçülen delta $\approx$0'dır. Pozitif delta yalnızca OFF senaryosu bant/çözünürlük baskısı altındayken
(küçük/uzak plaka ROI'si okunamaz piksele düştüğünde) ortaya çıkar ve daha agresif bir OFF
parametresiyle yeniden üretilir (örn. \tcode{docs/degerlendirme.md}: plaka +33pp). Dolayısıyla QoD'nin
katkısı, sabit ölçülmüş tek bir sayı değil, \textbf{mekanizmanın yönlülüğüdür}: \tcode{vehicle\_approach}
tetiği şartnamenin ``TOGG yaklaşınca QoD'' senaryosunu birebir karşılayıp kritik anda kaliteyi talep
eder; harness bu deltayı tek komutla ölçmeye hazırdır.
...
(Tablo~\ref{tab:fps}, Şekil~\ref{fig:fps}; \tcode{eval\_results/bench\_cuda0\_*.md}, 2026-06-26). CUDA,
MPS geliştirme alt-sınırının (\tcode{yolo26l} ${\sim}5,9$ FPS) \textbf{${\approx}2\times$}'i iş-hacmi sağlar.

\begin{table}[H]\centering\small
\caption{İşleme hızı — gerçek ölçüm (\tcode{eval\_results/bench\_cuda0\_*.md}, 2026-06-26).}\label{tab:fps}
...
sistem ${\sim}$12 FPS iş-hacmiyle 25--30 FPS'lik bir kamera akışını kare-örnekleme (her 2--3 karede bir)
ile olay-tespiti amacıyla işler; p95 gecikme zarfı bu kullanım için yeterlidir.

\begin{figure}[ht]\centering
\includegraphics[width=0.40\textwidth]{fig_fps.png}
...
\textbf{Hız/swerving.} Mutlak hız bu raporda \textbf{nicel olarak sınanmamıştır} (MAE/MAPE harness'ı
hazır; kalibrasyon ve gerçek-hız GT geldiğinde nicel hata üretir, yoksa dürüstçe mutlak hız iddia
edilmez). Sınanan yetenek kalibrasyonsuz swerving'dir: üç videodan \tcode{video\_3}'te gerçekten tespit
edilmiş ve davranış makro-F1'ine dahildir (Bölüm 4.3).
...
Ayrıca ${\sim}780$ testin bir bölümü CAMARA QoD/NV mock sözleşme uyumunu (sessiz doğrulama, oturum
yaşam-döngüsü) doğrular.

\textbf{Çözüme neden güveniyoruz?} Güven, ölçülen sayıların ötesinde \textbf{yapısal} gerekçelere
dayanır: tüm eşikler oran/boyut-temelli ve ölçek-bağımsızdır (videoya-özel sabit yok, K-004), dolayısıyla
sonuçlar tek çekime aşırı-uyumlanmaz; belirsizlikte sistem yanlış üretmek yerine dürüstçe çekimser kalır
(plakada \textbf{sıfır yanlış-onay}) ve bu güvenceler config-driven, gerçek videoda doğrulanmış zırhlara
(Bölüm 3.3) dayanır. Sınama kümesinin sınırı (üç-videoluk küçük set; istatistiksel mAP held-out sette
ayrıca ölçülmüştür, Tablo~\ref{tab:map}) açıkça belirtilmiş olup komite verisiyle güçlendirilecektir.

% ========================== 5. KAYNAKÇA ==============================
\section{Kaynakça}
{\footnotesize Başlıca kaynaklar (tam liste — 48 kaynak — açık veri lisansları ve izlenebilirlik tablosu: \tcode{kaynakca.md}):\par}
\footnotesize\sloppy
...
\item Yapay zekâ desteği: raporun yazım/düzenlenmesinde büyük dil modelinden (Anthropic Claude) destek alınmıştır; tüm teknik içerik ve sayılar takımca üretilip depo artefaktlarından doğrulanmıştır.
\end{enumerate}
\normalsize

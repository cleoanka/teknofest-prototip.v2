...
\begin{figure}[ht]\centering
\includegraphics[width=0.56\textwidth]{fig_mimari.png}
...
\begin{figure}[ht]\centering
\includegraphics[width=0.62\textwidth]{fig_tespit_map.png}
\caption{Held-out mAP@0.50 / mAP@0.50:0.95 (özel modeller domain held-out; \tcode{yolo26l} COCO genel set). Precision/Recall/F1 değerleri Tablo~\ref{tab:map}'dedir.}
\label{fig:map}
\end{figure}
...
(Şekil~\ref{fig:fps}; \tcode{eval\_results/bench\_cuda0\_*.md}, 2026-06-26). CUDA, MPS geliştirme
alt-sınırının (yolo26l ${\sim}5,9$ FPS) \textbf{${\approx}2\times$}'i throughput sağlar: \tcode{server}
profilinde (yolo26l, imgsz 960) \textbf{12,31 FPS} (p95 93 ms), \tcode{laptop} profilinde (yolo26s,
imgsz 640) \textbf{14,72 FPS} (p95 88 ms). \tcode{p95}=93 ms, 25--30 fps'lik trafik kamerası akışı için güvenli
SLA zarfıdır.

\begin{figure}[ht]\centering
\includegraphics[width=0.55\textwidth]{fig_fps.png}
\caption{İşleme hızı: MPS geliştirme alt-sınırı vs gerçek CUDA ölçümü (RTX 5070 Laptop GPU, 4.608 çekirdek, Compute 12.0).}
\label{fig:fps}
\end{figure}
...
\item Yapay zekâ desteği: bu raporun yazım ve düzenlenmesinde büyük dil modelinden (Anthropic Claude) destek alınmıştır. Tüm teknik içerik, mimari, ölçüm ve sayılar takım tarafından üretilmiş olup depo artefaktlarından (\tcode{eval\_results/}, \tcode{weights/}) doğrulanmıştır.
\end{enumerate}
\normalsize

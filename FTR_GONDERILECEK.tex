...
(3.104), \tcode{smoking} (557) ve \tcode{phone} (659, sentetik render). Her setin kaynağı, lisansı ve
...
\caption{AURA kuşbakışı kaskad boru hattı: Aşama-1 tespit/takipten iki ROI ile paralel Aşama-2 motorlarına (sürücü davranışı $\parallel$ plaka okuma) ve ID-merkezli birikime; QoD tetikleri olay akışından türetilir.}
\label{fig:mimari}
\end{figure}
...
\begin{table}[H]\centering\small
\caption{Held-out tespit doğruluğu (özel modeller ayrılmış test bölmesinde; stok \tcode{yolo26l} COCO val2017'de). $^{\dagger}$\,\tcode{license\_plate} üretimde varsayılan LP dedektörüdür; \tcode{seatbelt}/\tcode{smoking} fine-tune held-out sonuçlarıdır (üretim hattında opsiyonel/devre-dışı).}\label{tab:map}
...
\begin{table}[H]\centering\small
\caption{Davranış tespiti — üç gerçek test videosunda video-düzeyi P/R/F1 (her hücre P/R/F1).}\label{tab:beh}
...
metrikleri kıyaslanır. \textbf{Dürüst not (kritik):} mevcut kontrollü koşuda OFF baseline'ı zaten
yeterli kalitede olduğundan ölçülen delta $\approx$0'dır. Pozitif delta yalnızca OFF senaryosu
bant/çözünürlük baskısı altındayken (küçük/uzak plaka ROI'si okunamaz piksele düştüğünde) ortaya
çıkar ve daha agresif bir OFF parametresiyle yeniden üretilir (kontrollü senaryoda plaka okuma
\textbf{+33 puana} kadar iyileşir). Dolayısıyla QoD'nin katkısı, sabit ölçülmüş tek bir sayı değil,
\textbf{mekanizmanın yönlülüğüdür}: \tcode{vehicle\_approach} tetiği şartnamenin ``TOGG yaklaşınca
QoD'' senaryosunu birebir karşılayıp kritik anda 5G şebeke kalitesini talep eder.
...
(Tablo~\ref{tab:fps}, Şekil~\ref{fig:fps}; her profilde 100+ kare ölçümü, 26.06.2026). CUDA,
MPS geliştirme alt-sınırının (\tcode{yolo26l} ${\sim}5,9$ FPS) \textbf{${\approx}2\times$}'i iş-hacmi sağlar.

\begin{table}[H]\centering\small
\caption{İşleme hızı — RTX 5070 Laptop GPU üzerinde gerçek ölçüm (CUDA; 26.06.2026).}\label{tab:fps}
...
toplar. Böylece her sayı izlenebilir bir ölçüme dayanır ve rapora elle girilmemiştir. Ayrıca otomatik
test paketinin (${\sim}780$ test) bir bölümü CAMARA QoD/NV mock sözleşme uyumunu (sessiz doğrulama,
oturum yaşam-döngüsü) doğrular.
...
\item Yapay zekâ desteği: raporun yazım ve düzenlenmesinde büyük dil modeli tabanlı araçlardan yararlanılmıştır; tüm teknik içerik, mimari kararlar, ölçümler ve sayılar takım tarafından üretilmiş ve doğrulanmıştır.
\end{enumerate}
\normalsize

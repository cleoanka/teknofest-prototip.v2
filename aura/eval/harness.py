...
log = logging.getLogger("aura.eval")
...
def _run(cfg, source, scale: float = 1.0) -> dict:
    ...
    while True:
        ...
        small.append(sum(
            1 for t in anno.tracks
            if (t["bbox"][2] - t["bbox"][0]) * (t["bbox"][3] - t["bbox"][1]) < 0.02 * area
        ))
        i += 1
    cap.release()
    elapsed = time.time() - t0
    confirmed = {r.track_id: r.plate.value for r in pipe.acc.active_tracks()
                 if r.plate.status == "confirmed"}
    return {"confirmed": confirmed, "detected": detected, "small": small,
            "fps": round(i / elapsed, 1) if elapsed > 0 else 0.0, "frames": i}


def run_eval(cfg, source, ground_truth, qod_comparison: bool = True,
    ...
    metrics = [
        ...
        _m("Küçük nesne tespiti (%)", M.small_object_rate(off["small"], gt),
           M.small_object_rate(on["small"], gt)),
        _m("Tespit oranı (%)", M.detection_rate(off["detected"], gt),
           M.detection_rate(on["detected"], gt)),
    ]
    ts = time.strftime("%Y-%m-%d %H:%M:%S")
    report_md = _report_md(metrics, pa_on, pa_off, ts, source)
    result = {
        "timestamp": ts, "source": str(source), "qod_comparison": qod_comparison,
        "metrics": metrics, "plate_detail": {"qod_on": pa_on, "qod_off": pa_off},

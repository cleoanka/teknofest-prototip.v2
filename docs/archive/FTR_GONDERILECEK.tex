% =====================================================================
% RoadGuard — Final Tasarım Raporu (FTR) — GÖNDERİLECEK SÜRÜM
...
\setstretch{1.15}
\usepackage{xcolor}
\definecolor{rgmain}{HTML}{1b5e20}
\definecolor{rgblue}{HTML}{0d47a1}
\usepackage{titlesec}
\titleformat{\section}{\fontsize{14}{16}\bfseries\color{rgmain}}{\thesection.}{0.5em}{}
\titleformat{\subsection}{\fontsize{12}{14}\bfseries\color{rgblue}}{\thesubsection}{0.5em}{}
...
{\fontsize{42}{46}\bfseries\color{rgmain} RoadGuard}\\[3pt]
{\color{rgmain}\rule{0.5\textwidth}{1.2pt}}\\[12pt]
{\fontsize{15}{19}\bfseries 5G ve Yapay Zekâ ile Akıllı Yol Güvenliği Sistemi}\\[16pt]
{\fontsize{17}{21}\bfseries\color{rgblue} FİNAL TASARIM RAPORU}\\[2.6cm]

\begin{tabular}{rl}
\textbf{Takım Adı:} & {\large\bfseries\color{rgmain}Nankatsu} \\[12pt]
\textbf{Takım ID:} & {\large\bfseries\color{rgmain}\#985007} \\[12pt]
\textbf{Başvuru ID:} & {\large\bfseries\color{rgmain}\#5181367} \\[12pt]
...
% ============================ 1. ÖZET =================================
\section{Proje Özeti}
RoadGuard, yol kenarı trafik kamerası akışından \textbf{araç, plaka, hız ve riskli sürücü
...
telefon/sigara kullanımı, hız ihlali) gerçek zamanlı tespit etmek ve \textbf{yalnızca
...
GPU'da ölçülen \textbf{26--33 FPS} iş-hacmiyle kamera akışı gerçek zamanlı işlenmiştir.
...
adları RoadGuard kanonik taksonomisine eşlenir (ör. \tcode{cell phone}\,$\rightarrow$\,\tcode{phone},
...
Her sınıfın temsil dengesi eğitime girmeden önce nicel olarak ölçülmüştür: bir veri-denetim aracı,
her bölümün görüntü sayısını, sınıf-örnek dağılımını ve dengesizlik oranını (en kalabalık$/$en seyrek
sınıf) raporlar. Ölçülen oran \tcode{seatbelt} setinde \textbf{1,27}'dir (dengeli) ve setlerin görüntü
dağılımı Şekil~\ref{fig:denge}'de gösterilmiştir. Dengeleme iki koldan uygulanmıştır: (i) dengesizlik
oranı 3'ü aşan seyrek sınıflar saptanıp bu sınıflar lehine Ultralytics augmentasyonu (mozaik,
HSV-jitter, gece-gamma) güçlendirilmiş; (ii) az-temsil edilen davranış sınıflarını (kemer, sigara)
içeren görüntüler \textbf{yalnızca train bölümünde}, dengesizlik oranı 3'ün altına inene dek
çoğaltılarak \textbf{oversampling} uygulanmıştır (çoğaltma faktörü en çok 8$\times$; val/test'e
dokunulmaz, değerlendirme sızıntısı olmaz). Böylece nadir ihlaller baskın sınıflarca bastırılmadan
öğrenilmiştir.

\begin{figure}[ht]\centering
\begin{minipage}{0.47\textwidth}\centering\includegraphics[width=\textwidth]{fig_veri_dengesi.png}\end{minipage}\hfill
\begin{minipage}{0.47\textwidth}\centering\includegraphics[width=\textwidth]{fig_split.png}\end{minipage}
\caption{Sol: toplanan açık-kaynak setlerin görüntü dağılımı (tümü CC BY 4.0). Sağ: \tcode{license\_plate} setinin \%80/\%10/\%10 train/val/test bölünmesi (8.823 görsel).}
\label{fig:denge}\label{fig:split}
\end{figure}
...
parlatma uygulanır. \textbf{Overfitting kontrolü:} küçük özel setlerde aşırı-uyumu sınırlamak için
patience-tabanlı erken-durdurma, ağırlık-çürümesi ve \textbf{en-iyi-val checkpoint} dağıtımı (overfit
kuyruğu kullanılmaz) uygulanır; \tcode{license\_plate} eğitim/val mAP'i 0,989'da stabildir (aşırı-uyum
işareti yok), held-out test bölmesi ise bağımsızdır.

\subsection{Train / Val / Test Dağılımı}
Veri \textbf{\%80 train / \%10 val / \%10 test} oranında (seed 42) bölünmüştür (Şekil~\ref{fig:split});
görece küçük özel setlerde doğrulama ve test bölümlerinin istatistiksel anlam taşıması için her birine
\%10 pay ayrılmış, sınıf-dengesiz setlerde bölme tabakalı (stratified) yapılmıştır. Komite TOGG verisi
geldiğinde aynı boru hattı domain modelini tek komutla yeniden üretir.
...
içerir. RoadGuard, bu zorlukları gerçek footage üzerinde ölçerek tanımlamış ve her birine hedefli bir
...
RoadGuard'nın temel tercihi \textbf{kare-merkezli değil ID-merkezli} karar üretmektir — her araç bir
...
\caption{RoadGuard kuşbakışı kaskad boru hattı (soldan sağa akış): Aşama-1 tespit/takipten iki ROI ile üç paralel Aşama-2 motoruna (sürücü davranışı $\parallel$ plaka okuma $\parallel$ hız) ve ID-merkezli birikime; QoD tetikleyici (kesikli) kritik anda 5G kalitesini talep eder.}
\label{fig:mimari}
\end{figure}
...
büyük ROI'ler kırpılır. Karar eşikleri \textbf{videoya-özel sabit içermez} ve oran-bazlıdır; kalite
kapıları (asgari plaka/ROI yüksekliği) çözünürlük-ölçekli piksel minimumları kullanır.
...
uyumlu \textbf{talep-yönlü tetikleme ve kaynak verimliliği} olarak sunulur. Doğruluk-deltasını ve bant
tasarrufunu nicelemek için, üretim hattıyla hizalı üç-kollu A/B (Always-LOW / Always-HIGH / Adaptive-QoD)
ve küçük/uzak-plaka baskılı bir sentetik set, finale dek tamamlanacak somut iş kalemidir.
...
MPS geliştirme alt-sınırının (\tcode{yolo26l} $\sim$12,98 FPS) \textbf{${\approx}2\times$}'i iş-hacmi sağlar.
...
\tcode{server} · \tcode{yolo26l} · 960 (CUDA) & \textbf{27,08} & 36 & 42 \\
\tcode{laptop} · \tcode{yolo26s} · 640 (CUDA) & \textbf{32,38} & 30 & 40 \\
\tcode{server} · \tcode{yolo26l} (MPS, M4 Pro) & $\sim$12,98 & -- & -- \\
\bottomrule
\end{tabularx}
\end{table}

Kare-başına \textbf{gecikme} (p95=42 ms) ile \textbf{iş-hacmi} (${\sim}$27 FPS) ayrı büyüklüklerdir.
...
sistem ${\sim}$27 FPS iş-hacmiyle 25--30 FPS'lik bir kamera akışını gerçek-zamanlı işler; p95 gecikme
zarfı bu kullanım için fazlasıyla yeterlidir.
...
dayanır: karar eşikleri oran/boyut-temellidir ve \textbf{videoya-özel sabit içermez} (K-004; kalite kapıları çözünürlük-ölçeklidir), dolayısıyla

...
- **Config profil katmanı** (`config/profiles/*.yaml`, `default.yaml` üzerine derin-merge):
  ...
  `--profile` bayrağı (`roadguard`, `roadguard.eval`, `tools/test_video.py`) + `ROADGUARD_PROFILE` env.
...
- **FTR §4 metrik harness'ı** (`roadguard/eval/report.py`): `python -m roadguard.eval --metrics-report`
...
- **Sürücü KİMLİĞE değil POZİSYONA göre seçilir** (`roadguard/identity/driver_lock.py` yeniden yazıldı):
...
- **Plaka→hız oto-kalibrasyonu** (`feature/plate-speed-calibration`): LP plaka kutusu (520mm
  referans) `speed.observe_plate`'e beslenir → metrik ppm kalibrasyonu (`roadguard/speed/estimator.py`,
  `roadguard/plate/reader.py`, `tools/diag_speed_plate.py`, `tests/test_speed_plate_calib.py`).
...
### ✨ Eklenenler
- **Sürücü-içi sıkı kırpma** (`roadguard/driver_state/pose.py` → `_driver_crop`): pose + v4 nesne kanıtı artık tüm kabin yerine yalnız **sürücünün kişi kutusu (+%10)** üzerinde koşar (ön cam/yolcu yansımaları FP kaynağıydı). Kutu track başına önbelleğe alınır (`driver_crop.redetect_every`), kare başına tek pose geçişi korunur. ROI zaten darsa (`min_gain`) kırpma atlanır. Sürücü tarafı `driver_lock.corner` ile aynı sözleşmeyi kullanır.
...
- **Fine-tune dedektör** `weights/yolguvenligi_types_v4.pt` (11 sınıf, held-out test mAP50 .788; v1 reposundan Git LFS ile kurtarıldı) birincil detector yapıldı; yoksa stok `yolo26s.pt`'ye **loglu** fallback. Sınıf adı taksonomisi `roadguard/taxonomy.py` (`cell phone`→`phone`, `cigarette`→`smoking`...).
...
- **`tools/test_video.py`**: video → annotated mp4 + JSON özet (event sayıları, plaka oy dökümü, sürücü bayrak süreleri, swerving kareleri, QoD nedenleri) — jüri/denetim kanıtı (şartname 4.5). `python -m roadguard`'ya `--save-events` (JSONL kanıt izi).
...
### 🐛 Düzeltmeler
- **D3:** `--source` artık config'e yazılır — `ai_mode: auto` gerçek videoyu mock'ta işleyebiliyordu (`roadguard/__main__.py` + `services/.../state.py`).
- **D4:** `--device auto` macOS'ta artık MPS seçer (önceden hep CPU'ya düşüyordu, `roadguard/device.py`).
...
#### ✨ Eklendi
- **Plaka OCR varsayılanı `fast-plate-ocr`** (`roadguard/plate/ocr.py`, `ocr_engine: fastplate`): plakaya-özel
...
- **mAP harness ölçüldü** (`roadguard/eval/map_eval.py`, `python -m roadguard.eval --map`): stok `yolo26l`
...
- Tam monorepo dizin iskeleti oluşturuldu (`roadguard/`, `services/`, `dashboard/`, `train/`, `docs/`, `tests/`).
...
- `pyproject.toml` (roadguard paketi + core/dev bağımlılık grupları), `Makefile`, `.gitignore`, `.gitattributes`, `.env.example`, `LICENSE` (MIT).
...
- `roadguard/schema.py`: §6.0 sözleşmeleri (`PlateState`, `DriverState`, `SpeedState`, `BBox`, `TrackRecord`, `RoadGuardEvent`, `AnnotationFrame`) + `make_event` yardımcısı.
...
- `python -m roadguard` CLI (§4.1 argparse) + `Pipeline.run_video/frames`.
...
- `roadguard/detection/yolo.py`: `YOLO26Detector` (gerçek) — ultralytics YOLO26 + ByteTrack, araç sınıf filtresi, ROI crop.
- `roadguard/detection/mock.py`: `MockDetector` (deterministik) — parlak araç bloklarını eşikler + `SimpleIoUTracker` ile kalıcı ID; ağırlık olmadan tüm hat çalışır.
...
- `roadguard/stability/state_machine.py` (gerçek): per `track×alan` kayar pencere (16), ≥8 tutarlılıkta commit; flicker'da önceki yüksek-güvenli değer korunur.
- `roadguard/driver_state/yolo.py`: `YOLO26lDriverClassifier` (gerçek) — cabin ROI çoklu-etiket detection.
- `roadguard/driver_state/mock.py`: `MockDriverClassifier` — cabin baskın rengini senaryo durumuna eşler (phone / smoking+no_seatbelt / fatigue).
...
- `roadguard/plate/ocr.py`: `RealOCR` (EasyOCR) + `MockOCR` (renk→senaryo plakası); `build_ocr` çözümlemesi.
- `roadguard/plate/voting.py`: `VotingBuffer` (konsensüs/ret).
- `roadguard/plate/reader.py` (gerçek): sweet-spot gating → voting → Türk plaka regex → konsensüs (PLATE_CONFIRMED + erken çıkış) / ret (PLATE_REJECTED + QoD kalite tetiği + yeniden okuma); yetersiz piksel → QoD.
- `roadguard/qod/client.py`: `QoDController` — histerezis (min_active + cooldown), QOD_TRIGGER/RELEASE; pipeline'a entegre.
...
- `roadguard/speed/estimator.py` (gerçek): `disabled` (relative_velocity_flag), `tripwire` (iki çizgi × gerçek mesafe / frame-delta), `ipm` (M12 opsiyonel modüle güvenli düşüş).
...
- `GET /cameras`: OpenCV enum + platform isim çözümleme (macOS AVFoundation), `ROADGUARD_CAMERA_PROBE=0` ile atlanır.
...
- `roadguard/eval/metrics.py`: Levenshtein/CER, plaka doğruluğu, tespit oranı, küçük-nesne oranı.
- `roadguard/eval/harness.py`: aynı video QoD ON (tam çözünürlük) vs QoD OFF (düşük çözünürlük) → GT'ye karşı delta tablosu + report.md/json.
- `python -m roadguard.eval` CLI (§4.3) + `/eval/run`/`/eval/results`/`/eval/results/export` bağlandı; dashboard Chart.js paneli tüketir.
...
- `roadguard/optional/loader.py`: `get_optional(cfg, name)` — flag kapalıyken **import yapmaz** (lazy).
